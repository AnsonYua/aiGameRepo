...
- `play_card` 指令的格式依卡牌類型不同：基地卡(Base) → `play_card <card_id>`（無插槽）；單位卡(Unit) → `play_card <card_id> <slot>`（需插槽號碼）；配對(Pilot) → `pair <card_id> my_slot_<n>`。不同 cardType 的 `play_card` 不能跨區域使用，legal_commands 中沒有包含該 card_id 的指令時，代表該卡當前無法合法使用。
...
4. 進攻優先序：攻擊對手基地/盾牌/玩家 > 有利擊殺 rested 威脅 > 清 Blocker > 部署下回合壓力。若對手基地 HP 極低（≤2），攻擊基地摧毀它優先於擊殺任何 rested 單位：移除基地等於移除整層防禦層，推進程度遠高於擊殺一個攻擊者。
