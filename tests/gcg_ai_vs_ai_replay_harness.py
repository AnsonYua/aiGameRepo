...
def choose_fake_command(player_id: str) -> str:
    state = load_state(active_game_id())
...
def write_review(game_id: str, analysis: dict[str, Any], opencode_calls: int, live_llm: bool) -> Path:
    ...
    lines = [
        ...
        f"Replay/log quality: gameplay.yaml parsed, replay.md present, opencode_agent_calls={opencode_calls}, live_llm={live_llm}",
        "Problems:",
        *[f"- {problem}" for problem in problems],
        "Likely root cause:",
        "- 若缺少 attack unit / block，優先檢查 gcg-ai-player.md 是否從 display 看得到該窗口與目標；其次檢查 runtime 是否提供合法 command surface。",
        "Follow-up:",
    ...
    try:
        ...
        if args.live_llm:
            opencode_call_count = sum(
                ...
                if event.get("event_type") == "decision_received" and event.get("actor") in {"P1", "P2"}
            )
        else:
