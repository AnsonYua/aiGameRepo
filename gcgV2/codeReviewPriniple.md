...
- keep control flow lean and straightforward; prefer branching directly on existing game state instead of adding patch variables only to steer one path
- do not add extra variables or flags that are used only once; if the logic can be expressed directly at the call site or from existing state, do that instead
