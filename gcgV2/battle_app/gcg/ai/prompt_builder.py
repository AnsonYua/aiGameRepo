...
_STRATEGY_NOTES = {
    "main": [
        ...
        "高機動單位不會被阻擋；宣告攻擊後會跳過阻擋步驟，適合直接推進防禦層或安全攻擊 rested 單位。",
        "先制攻擊單位在戰鬥中先造成傷害。若先制單位的 AP ≥ 對手剩餘 HP，對手被擊破且先制單位不受反傷；若雙方都有先制，仍同時結算。",
        "貫通 N 只在攻擊者於戰鬥中擊破阻擋者或被攻擊單位後生效，會對對手盾牌或玩家造成 N 點額外傷害。",
    ...
    "block": [
        ...
        "高機動攻擊者不會進入阻擋步驟；若你正在阻擋步驟，代表這次攻擊者不是高機動或規則允許阻擋。",
        "面對先制攻擊者時，若我方 Blocker 會在先制傷害中先被擊破，就不會造成反傷；阻擋前要看 `block_annotations`。",
    ...
    "action": [
        ...
        "Action 卡若能授予先制、貫通、傷害防止或攻擊限制，優先確認這些狀態是否會立刻改變本次戰鬥結果。",
...
class PromptBuilder:
    ...
    def _annotate_attacks(self, legal_commands, viewer_state):
        ...
        for command in legal_commands:
            ...
            if self._has_keyword(attacker, "High-Maneuver"):
                entry["block_step"] = "高機動：此攻擊不會被阻擋，會跳過阻擋步驟。"
            else:
                blocked_lines = [
    ...
    def _annotate_blocks(self, legal_commands, viewer_state):
        ...
        for command in legal_commands:
            ...
            if blocker is None:
                continue
            fight = self._preview_unit_fight(
                attacker,
                blocker,
                attacker_label="對手攻擊者",
                defender_label="我方 Blocker",
            )
            annotations[command] = (
                f"我方 {blocker['slot']} 號位（{blocker.get('ap', 0)}/{blocker.get('remaining_hp', 0)}）"
                f"阻擋對手攻擊者 {attacker['slot']} 號位"
                f"（{attacker.get('ap', 0)}/{attacker.get('remaining_hp', 0)}）：{fight}"
            )
        return annotations
    ...
    def _preview_unit_fight(self, attacker, defender, attacker_label="我方", defender_label="對手"):
        ...
        attacker_name = f"{attacker_label} {attacker['slot']} 號位"
        defender_name = f"{defender_label} {defender['slot']} 號位"

        if attacker_first and not defender_first and my_ap >= def_hp:
            breach = self._breach_preview(attacker, defender_dies=True)
            return (
                f"先制攻擊：{defender_name}被擊破；"
                f"{attacker_name}存活（無反傷）{breach}"
            )
        if defender_first and not attacker_first and def_ap >= my_hp:
            return (
                f"{defender_label}先制攻擊：{attacker_name}被擊破；"
                f"{defender_name}存活（{attacker_label}無法反傷）"
            )
        ...
        prefix = "雙方先制同時結算：" if attacker_first and defender_first else "互相造成傷害："
        breach = self._breach_preview(attacker, defender_dies=defender_dies)
        return (
            f"{prefix}{defender_name} {defender_text}；"
            f"{attacker_name} {attacker_text}{breach}"
        )

    def _keywords(self, slot):
        return list(slot.get("keywords") or [])

    def _has_keyword(self, slot, keyword):
        return keyword in self._keywords(slot)

    def _breach_damage(self, slot):
        for keyword in self._keywords(slot):
            if str(keyword).startswith("Breach:"):
                return int(str(keyword).split(":", 1)[1] or 0)
        return 0

    def _breach_preview(self, attacker, defender_dies):
        damage = self._breach_damage(attacker)
        if not defender_dies or damage <= 0:
            return ""
        return f"；貫通 {damage} → 對手盾牌/玩家額外受到 {damage} 傷害"
    ...
    def _lesson_features(self, viewer_state):
        ...
        return {
            ...
            "enemy_has_breach": any(self._breach_damage(slot) > 0 for slot in opp_slots),
            "enemy_has_first_strike": any(self._has_keyword(slot, "First Strike") for slot in opp_slots),
            "enemy_has_high_mobility": any(self._has_keyword(slot, "High-Maneuver") for slot in opp_slots),
            "my_units_have_granted_keyword": any(
                slot.get("temp_keywords") or slot.get("cont_keywords") for slot in my_slots
            ),
            "my_units_have_cannot_attack": any(
                slot.get("temp_cannot_attack")
                or slot.get("cont_cannot_attack")
                or slot.get("temp_cannot_attack_player")
                or slot.get("cont_cannot_attack_player")
    ...
    def _has_attack_restricted_unit(self, my_slots):
        for slot in my_slots:
            if (slot.get("temp_cannot_attack")
                    or slot.get("cont_cannot_attack")
                    or slot.get("temp_cannot_attack_player")
                    or slot.get("cont_cannot_attack_player")):
                return True
