...
class RulesIndex:
    def __init__(self, card_database, schema_index=None):
    ...
    def info(self, card_id):
        ...
        schema_info = self.schema_index.info(normalized) if self.schema_index is not None else None
        if schema_info is not None:
            info.update({
                "keywords": list(schema_info.keywords),
    ...
    def has_activated_action(self, card_id):
        return self.info(card_id)["has_activated_action"]
    ...
    def _build_info(self, card):
        ...
        if support_match:
            ...
            support_keyword = f"Support:{int(support_match.group(1) or 0)}"
            if support_keyword not in keywords:
                keywords.append(support_keyword)
        ...
        if re.search(r"\[Activate/Main\]", text):
            has_activated_main = True
        if re.search(r"\[Activate/Action\]", text):
            has_activated_action = True
