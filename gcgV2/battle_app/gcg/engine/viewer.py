...
class ViewerStateBuilder:
    ...
    def _build_player_block(self, player_state, is_viewer):
        ...
        visible_battle_area = [self._build_battle_slot(slot) for slot in battle_area]
        return {
            ...
            "battle_area": visible_battle_area,
            "board_summary": {
                "units": sum(1 for slot in visible_battle_area if slot.get("unit_id") is not None),
                "empty_slots": sum(1 for slot in visible_battle_area if slot.get("unit_id") is None),
                "rested_units": sum(1 for slot in visible_battle_area if slot.get("status") == "rested"),
                "damaged_units": sum(1 for slot in visible_battle_area if slot.get("damage", 0) > 0),
                "blockers": sum(1 for slot in visible_battle_area if "Blocker" in slot.get("keywords", [])),
            },
    ...
    def _build_battle_slot(self, slot):
        ...
        base_keywords = list(slot.get("keywords", []))
        temp_keywords = list(slot.get("temp_keywords", []))
        cont_keywords = list(slot.get("cont_keywords", []))
        return {
            "slot": slot.get("slot"),
            ...
            "keywords": self._merge_keywords(base_keywords, temp_keywords, cont_keywords),
            ...
            "granted_triggers_summary": self._granted_triggers_summary(slot),
            "turns_on_field": slot.get("turns_on_field", 0),
        }

    def _merge_keywords(self, *keyword_lists):
        merged = []
        ...
        if valued_prefixes:
            merged = [
                keyword for keyword in merged
                if not (isinstance(keyword, str) and ":" not in keyword and keyword in valued_prefixes)
            ]
        return merged

    def _granted_triggers_summary(self, slot):
        summary = []
        for granted in slot.get("granted_triggers") or []:
            timing = granted.get("timing")
            source = granted.get("source_card_id")
            if timing and source:
                summary.append(f"{timing}:{source}")
            elif timing:
                summary.append(str(timing))
        return summary
