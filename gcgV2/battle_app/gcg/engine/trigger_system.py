"""Trigger detection and queue management.

偵測（deterministic，靠 RulesIndex）→ 入 queue → runtime resolve loop 逐一
交給 interpreter 解讀並執行。
"""

from __future__ import annotations

from copy import deepcopy


def make_trigger(timing, controller, card_id, source_slot=None, source_zone="battle_area", after_default=None):
    return {
        "timing": timing,
        "controller": controller,
        "card_id": card_id,
        "source_slot": source_slot,
        "source_zone": source_zone,
        # trigger 結算後來源卡的預設去向（例如 burst 卡未被效果移動 → trash）
        "after_default": after_default,
    }


class TriggerSystem:
    def __init__(self, state_store, rules_index):
        self.state = state_store
        self.rules_index = rules_index

    # ------------------------------------------------------------------
    # detection from events
    # ------------------------------------------------------------------

    def detect_for_event(self, event):
        """把一個 engine 事件轉成 0..n 個 trigger context。"""
        event_type = event.get("type")
        if event_type == "unit_deployed":
            return self._unit_enters_play(event)
        if event_type == "base_deployed":
            return self._base_enters_play(event)
        if event_type == "pairing_complete":
            return self._pairing_complete(event)
        if event_type == "attack_declared":
            return self._attack_declared(event)
        if event_type == "unit_destroyed":
            return self._unit_destroyed(event)
        if event_type == "shield_broken":
            return self._shield_broken(event)
        return []

    def detect_end_of_turn(self, active_player):
        """回合結束觸發（例如 <Repair>），只看 active player 的場面。"""
        triggers = []
        for slot in self.state.iter_units(active_player):
            if self.rules_index.has_trigger(slot["unit_id"], "END_OF_TURN"):
                triggers.append(make_trigger(
                    timing="END_OF_TURN",
                    controller=active_player,
                    card_id=slot["unit_id"],
                    source_slot=slot["slot"],
                ))
        return triggers

    # ------------------------------------------------------------------

    def _unit_enters_play(self, event):
        card_id = event["card_id"]
        if not self.rules_index.has_trigger(card_id, "ENTERS_PLAY"):
            return []
        return [make_trigger(
            timing="ENTERS_PLAY",
            controller=event["player"],
            card_id=card_id,
            source_slot=event["slot"],
        )]

    def _base_enters_play(self, event):
        card_id = event["card_id"]
        if not self.rules_index.has_trigger(card_id, "ENTERS_PLAY"):
            return []
        return [make_trigger(
            timing="ENTERS_PLAY",
            controller=event["player"],
            card_id=card_id,
            source_zone="base",
        )]

    def _pairing_complete(self, event):
        triggers = []
        slot_index = event["slot"]
        player = event["player"]
        unit_id = event.get("unit_id")
        pilot_id = event.get("pilot_id")
        # [When Paired] 可能寫在 Unit 卡或 Pilot 卡上
        if unit_id and self.rules_index.has_trigger(unit_id, "PAIRING_COMPLETE"):
            triggers.append(make_trigger(
                timing="PAIRING_COMPLETE",
                controller=player,
                card_id=unit_id,
                source_slot=slot_index,
            ))
        if pilot_id and self.rules_index.has_trigger(pilot_id, "PAIRING_COMPLETE"):
            triggers.append(make_trigger(
                timing="PAIRING_COMPLETE",
                controller=player,
                card_id=pilot_id,
                source_slot=slot_index,
            ))
        slot = self.state.get_slot(player, slot_index)
        if slot.get("is_link"):
            for card_id in (unit_id, pilot_id):
                if card_id and self.rules_index.has_trigger(card_id, "ON_RESONANCE"):
                    triggers.append(make_trigger(
                        timing="ON_RESONANCE",
                        controller=player,
                        card_id=card_id,
                        source_slot=slot_index,
                    ))
        return triggers

    def _attack_declared(self, event):
        triggers = []
        player = event["player"]
        slot_index = event["slot"]
        slot = self.state.get_slot(player, slot_index)
        unit_id = slot.get("unit_id")
        pilot_id = slot.get("pilot_id")
        if unit_id and self.rules_index.has_trigger(unit_id, "ATTACK_PHASE"):
            triggers.append(make_trigger(
                timing="ATTACK_PHASE",
                controller=player,
                card_id=unit_id,
                source_slot=slot_index,
            ))
        if pilot_id and self.rules_index.has_trigger(pilot_id, "ATTACK_PHASE"):
            triggers.append(make_trigger(
                timing="ATTACK_PHASE",
                controller=player,
                card_id=pilot_id,
                source_slot=slot_index,
            ))
        return triggers

    def _unit_destroyed(self, event):
        triggers = []
        card_id = event["card_id"]
        if self.rules_index.has_trigger(card_id, "DESTROYED"):
            triggers.append(make_trigger(
                timing="DESTROYED",
                controller=event["player"],
                card_id=card_id,
                source_slot=None,
                source_zone="trash",
            ))
        if event.get("destroyed_by_battle"):
            triggers.extend(self._battle_destroyed(event))
        return triggers

    def _battle_destroyed(self, event):
        triggers = []
        source_player = event.get("source_player")
        source_slot = event.get("source_slot")
        if source_player is None:
            return triggers
        source_card_ids = event.get("source_card_ids")
        if source_card_ids is None and source_slot is not None:
            source = self.state.get_slot(source_player, source_slot)
            source_card_ids = [source.get("unit_id"), source.get("pilot_id")]
        for card_id in source_card_ids or []:
            if card_id and self.rules_index.has_trigger(card_id, "BATTLE_DESTROY"):
                triggers.append(make_trigger(
                    timing="BATTLE_DESTROY",
                    controller=source_player,
                    card_id=card_id,
                    source_slot=source_slot,
                ))
        triggers.extend(self._granted_triggers_for_event(event, source_player, source_slot, "BATTLE_DESTROY"))
        return triggers

    def _shield_broken(self, event):
        """盾牌破壞：有 [Burst] → burst trigger；否則由 runtime 直接進廢棄區。"""
        triggers = []
        source_player = event.get("source_player")
        source_slot = event.get("source_slot")
        if source_player is not None:
            source_card_ids = event.get("source_card_ids")
            if source_card_ids is None and source_slot is not None:
                source = self.state.get_slot(source_player, source_slot)
                source_card_ids = [source.get("unit_id"), source.get("pilot_id")]
            for source_card_id in source_card_ids or []:
                if source_card_id and self.rules_index.has_trigger(source_card_id, "ON_SHIELD_BREAK"):
                    triggers.append(make_trigger(
                        timing="ON_SHIELD_BREAK",
                        controller=source_player,
                        card_id=source_card_id,
                        source_slot=source_slot,
                    ))
            triggers.extend(self._granted_triggers_for_event(event, source_player, source_slot, "ON_SHIELD_BREAK"))
        card_id = event["card_id"]
        if not self.rules_index.has_trigger(card_id, "BURST_CONDITION"):
            return triggers
        triggers.append(make_trigger(
            timing="BURST_CONDITION",
            controller=event["player"],
            card_id=card_id,
            source_zone="burst",
            after_default="trash",
        ))
        return triggers

    def _granted_triggers_for_event(self, event, source_player, source_slot, timing):
        triggers = []
        for granted in event.get("source_granted_triggers") or []:
            if granted.get("timing") != timing:
                continue
            if not self._granted_condition_matches(granted.get("condition"), event, source_player):
                continue
            trigger = make_trigger(
                timing=timing,
                controller=source_player,
                card_id=granted.get("source_card_id"),
                source_slot=source_slot,
            )
            trigger["spec"] = deepcopy(granted.get("spec"))
            triggers.append(trigger)
        return triggers

    def _granted_condition_matches(self, condition, event, source_player):
        if condition in (None, "none"):
            return True
        if not isinstance(condition, dict):
            return False
        if "all_of" in condition:
            return all(
                self._granted_condition_matches(child, event, source_player)
                for child in condition.get("all_of") or []
            )
        condition_type = condition.get("type")
        if condition_type == "event_source_is":
            return condition.get("source") in {"this_unit", "source"}
        if condition_type == "destroyed_unit_controller_is":
            controller = condition.get("controller")
            if controller == "opponent":
                return event.get("player") == self.state.get_other_player(source_player)
            if controller == "self":
                return event.get("player") == source_player
        return False

    # ------------------------------------------------------------------
    # queue helpers
    # ------------------------------------------------------------------

    def enqueue_all(self, triggers):
        ordered = self._ordered_triggers(triggers)
        if not ordered:
            return
        if self.state.has_trigger():
            existing = self.state.list_trigger_queue()
            merged = (
                self._burst_triggers(ordered)
                + self._burst_triggers(existing)
                + self._non_burst_triggers(ordered)
                + self._non_burst_triggers(existing)
            )
            self.state.merge_trigger_queue(merged)
            return
        for trigger in ordered:
            self.state.enqueue_trigger(trigger)

    def _ordered_triggers(self, triggers):
        active_player = self.state.get_active_player()

        def priority(trigger):
            burst_rank = 0 if trigger.get("timing") == "BURST_CONDITION" else 1
            active_rank = 0 if trigger.get("controller") == active_player else 1
            return (burst_rank, active_rank)

        return sorted(list(triggers), key=priority)

    def _burst_triggers(self, triggers):
        return [trigger for trigger in triggers if trigger.get("timing") == "BURST_CONDITION"]

    def _non_burst_triggers(self, triggers):
        return [trigger for trigger in triggers if trigger.get("timing") != "BURST_CONDITION"]

    def has_waiting_trigger(self):
        return self.state.has_trigger()

    def pop_next_trigger(self):
        return self.state.pop_next_trigger()
