...
from ..effects.schema_interpreter import SchemaEffectInterpreter
from ..effects.schema_loader import CardEffectSchemaLoader
...
def build_simulator(
    ...
    players: "llm"（正式）、"scripted"（測試/離線）、「mcts」或 "hermes"（Hermes Agent）
    interpreter: "llm"（正式）、「schema」（結構化效果）或 "reference"（測試/離線，僅 ST01）
    """
    ...
    elif interpreter == "schema":
        effect_interpreter = SchemaEffectInterpreter(dictionary, schema_index=schema_index)
    else:
    ...
    if players == "llm":
        ...
        from ..ai.mcts.config import MctsConfig

        seed = os.getenv("GCG_MCTS_SEED")
        rng = random.Random(int(seed)) if seed is not None else random
        mcts_config = MctsConfig.from_env()
        mcts_kwargs = {
            "card_database": card_database,
            "deck_config": deck_config,
            "rules_index": rules_index,
            ...
            "max_rollout_depth": mcts_config.max_rollout_depth,
            "epsilon": mcts_config.epsilon,
            "time_budget_ms": mcts_config.time_budget_ms,
            "deck_id": {"P1": "deck001", "P2": "deck001"},
            "rng": rng,
        }
        player_map = {player_id: MctsPlayer(**mcts_kwargs) for player_id in ("P1", "P2")}
    elif players == "hermes":
        player_map = {
