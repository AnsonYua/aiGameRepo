...
_STEP_KEYS = {
    ...
    "else_steps", "ability", "tokens", "notes", "keyword", "value",
    "cannot_attack", "cannot_target_player", "attack_target_filter",
    "token", "filter", "count", "to", "remainder", "reveal",
    "source_filter", "damage_type",
}
...
class SpecGate:
    ...
    def _validate_step(self, step, prefix, names):
        ...
        if condition is not None:
            if not isinstance(condition, dict):
                ...
                errors.extend(self._validate_condition(condition, prefix))
    ...
    def _validate_condition(self, condition, prefix):
        errors = []
        condition_type = condition.get("type")
        if condition_type not in SUPPORTED_CONDITIONS:
            return [f"{prefix} unsupported condition type: {condition_type!r}"]
        if condition_type == "all_of":
            conditions = condition.get("conditions")
            if not isinstance(conditions, list) or not conditions:
                return [f"{prefix} all_of requires non-empty conditions"]
            for index, child in enumerate(conditions):
                if not isinstance(child, dict):
                    errors.append(f"{prefix}.conditions[{index}] must be a dict")
                    continue
                errors.extend(self._validate_condition(child, f"{prefix}.conditions[{index}]"))
        if condition_type == "not":
            child = condition.get("condition")
            if not isinstance(child, dict):
                return [f"{prefix} not requires condition"]
            errors.extend(self._validate_condition(child, f"{prefix}.condition"))
        return errors
    ...
    def _validate_target_ref(self, target, prefix, names):
        ...
        if target in _SPECIAL_TARGETS:
            return []
        if target.startswith(("self_all_unit_", "opponent_all_unit_")):
            return []
